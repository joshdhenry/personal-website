...
## Overview
A single-page personal site for Josh Henry, a Senior Mobile Software Engineer in Portland, OR. It is a portfolio and job-search site aimed at hiring managers, engineers, recruiters, and general visitors. Sections in order: hero, projects, skills, experience, about, contact, footer.

The site is intended to be built in **React Native + Expo with React Native Web**, so the same codebase runs on iOS, Android, and the web. The site being a working cross-platform app is itself the portfolio argument, and the footer says so.

## About the design files
`joshhenry.info.dc.html` in this bundle is a **design reference created in HTML**. It is a prototype showing intended look and behavior. It is not production code to copy.

The task is to **recreate this design in a React Native + Expo codebase** using that environment's patterns: React Navigation or plain scroll views, Reanimated for animation, StyleSheet or your styling library of choice. Nothing in the HTML should be lifted verbatim.

## Fidelity
**High fidelity.** Colors, typography, spacing, copy, and interactions are final. Recreate the UI faithfully. Two caveats:

1. The design is authored at desktop width only. Mobile and tablet breakpoints are **not** designed and need design decisions during the build. See "Responsive" below.
2. Hover states are web-only. Native needs press equivalents.
...
| border-dashed | `#D6DAE1` | placeholder bands |
...
| muted-lighter | `#B6BDC7` | placeholder subtext |
...
| amber-bg / amber-border / amber-text | `rgba(245,158,11,.12)` / `rgba(245,158,11,.3)` / `#B45309` | TODO chips (none remain, kept for reference) |
...
- Log area, fixed **224px** height, `overflow: hidden`, 20px horizontal padding. Seven rows, each: muted `·`, tech name in ink 500, a dotted leader that flexes to fill, and the years right-aligned in muted. Rows fade and rise in on staggered delays from 1.5s to 3.9s in 0.4s steps.
...
The seven log rows, in order: JavaScript / TypeScript 8 yrs, React Native 6.25 yrs, Swift 5.5 yrs, Objective-C 5.5 yrs, Java 4.5 yrs, Kotlin 3.5 yrs, React 2 yrs.
...
Card anatomy:
- Image band on top: `aspect-ratio: 7/3`, white, 1px border, 8px radius, 22px bottom margin, `overflow: hidden`. Image is `object-fit: contain`, 7px radius. The band ratio matches the source art (700x300) so `contain` produces no letterbox. **The Streem card is the exception**: its band carries `padding: 58px 27%` because the asset is a wide wordmark that must never be cropped or oversized.
...
Ten groups: Languages, React Native, UI / UX, Media, Data & State, Testing, Build & Release, Tools, Methodologies, AI. Full contents are in the logic class's `skillGroups` array.
...
The five roles, newest first: Streem 2023 to 2026 (Remote), Fall Guy Consulting 2021 to 2022 (Hybrid), Committee for Children 2020 to 2021 (Hybrid), Fall Guy Consulting 2017 to 2020 (Hybrid), Big Smash Software 2016 to 2017.
...
Left column is centered, 18px gap: "Let's build something great", a paragraph stating remote in the U.S. or hybrid in Portland, then three stacked badges at max-width 320px: LinkedIn, GitHub, resume.pdf. **The email address appears nowhere on the page, by request.** All contact goes through the form, so scrapers have no address to harvest. Do not add a mailto link. Each badge is white, 1px border, 8px radius, 11px/16px padding, an 18px inline SVG logo, then mono 0.8125rem text. Hover lifts 2px with indigo border and text.
...
Two: the sticky nav appearing past 560px, and a parallax on the terminal card (`translateY` clamped to ±28px at `(scrollY - 120) * -0.055`). On web these read `window.scrollY` or the nearest scrollable ancestor. On native, drive both from a shared scroll value via `useAnimatedScrollHandler`.
...
### Form
Posts to Formspree at `https://formspree.io/f/xljrgjpe` (free plan, 50 submissions a month). Verify it works from the production domain, as Formspree filters by referrer. There is no client-side validation in the design; add at minimum a required-email check and success and error states, neither of which is designed yet.
...
### Still open
Touch targets on native must be at least 44px. The card expand affordance is a small text label; make the entire card pressable, as it is on web.

Touch targets on native must be at least 44px. The compact card expand affordance is currently a small text label; make the whole card the target, as it is on web.
...
## Assets
All in `uploads/` in this project and copied into this bundle's `assets/` folder.

| File | Used by |
| --- | --- |
| `portfolio-streem.jpg` | Streem card (wordmark, do not crop) |
| `portfolio-gencon.png` | Gen Con card. The tagline on the right was originally white on transparent and invisible on a white card; it has been recolored to `#2C2A25` to match the logo block. Use this file, not the original. |
| `portfolio-mindYeti.png` | Mind Yeti card |
| `portfolio-player.png` | Second Step K-8 card |
| `portfolio-sela.png` | Second Step SEL for Adults card |
| `portfolio-diabloGolf.png` | Diablo Golf card |
| `portfolio-my-fish-pal.jpg` | My Fish Pal card |
| `portfolio-zen-builder.jpg` | Zen Builder card |
| `portfolio-vista-weather.jpg` | Vista Weather card |
| `portfolio-skyline-trivia.jpg` | Skylines Trivia card |
| `portfolio-spinwheelcontrol.jpg` | SpinWheelControl card |
| `headshot.png` | About section |
| `Josh Henry - Portfolio Resume.pdf` | Resume badges in hero and contact |

The portfolio PNGs are 700x300, which is why image bands use a 7/3 aspect ratio. The Streem wordmark is 480x144 and is padded rather than fitted.

Icons are inline SVG: an envelope drawn in indigo strokes, the GitHub mark in `#14181F`, the LinkedIn mark in `#0A66C2`, and a document outline for the resume badge. Use `react-native-svg`.

## Copy rules
From the project's CLAUDE.md, and they matter:
- **Never use em dashes.** Commas, colons, parentheses, or separate sentences instead. The whole site was scrubbed of them.
- Plain and factual tone. First person in the hero intro and About.
- **Do not rewrite copy Josh supplied.** Several project descriptions and the hero intro are his words verbatim.

## Build order suggestion
1. Scaffold Expo + React Native Web, load the three fonts, and set up the token constants.
2. Build the static page top to bottom with no animation: hero, projects, skills, experience, about, contact, footer.
3. Add `isNarrow` from `useWindowDimensions()` and branch the layouts.
4. Add interaction: card expand state, then Pressable hover/press states.
5. Add motion last: entrance animations, the counters, the typing terminal, the timeline rail draw, and the scroll-driven nav and parallax.

Motion last matters. The HTML went through three rounds of breakage from animating before the layout was settled, each time leaving parts of the page invisible.

## Files
- `joshhenry.info.dc.html` — the complete design. Open it in a browser to see every animation and interaction.
- `COPY_RULES.md` — project copy rules and the canonical fact list (years of experience, contact details, section order).
- `resume.txt` — extracted resume text, the source for experience and project detail.
